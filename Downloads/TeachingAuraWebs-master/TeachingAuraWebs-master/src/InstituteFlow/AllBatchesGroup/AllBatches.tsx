...
const AllBatches = (props) => {
  ...
  const handling = async () => {
    setAllBatches(
      (await client.get(`/institute/${signInDetails.id}/batch`)).data.filter(
        function (itr) {
    ...
    setcourseDetails(
      (await client.get(`/institute/${signInDetails.id}/course/${courseId}`))
        .data
    );
  ...
  const getData = async () => {
    ...
    courseId === undefined
      ? setAllBatches(
          (await client.get(`/institute/${signInDetails.id}/batch`)).data
        )
      : await handling();
  };
  ...
  const CategoriseData = () => {
    ...
    return {
      ...
      LiveCourses: Array.from(
        new Set(
          AllBatches.filter(function (itr) {
            return itr["startDate"] <= Date.now();
          }).map((itr) => itr["courseId"])
        )
      ),
      UpcomingCourses: Array.from(
        new Set(
          AllBatches.filter(function (itr) {
            return itr["startDate"] > Date.now();
          }).map((itr) => itr["courseId"])
  ...
  const courseIds = Array.from(
    new Set(AllBatches.map((itr) => itr["courseId"]))
  );
  ...
  return (
    <div className="content_full_container box_ladto">
      <Side_Menu active={courseId === undefined ? "batches" : ""} />
        ...
        <header className="heder_nav  ">
          <div className="box_vc_nav_right box_vc_nav_right22">
            {courseId !== undefined ? (
              <a
                onClick={() => navigate(-1)}
                // style={{visibility: (courseId === undefined) ? "hidden" : "visible"}}
              >
              ...
              <div className="teit_all">
                ...
                <div className="box_vd_s_continer">
                  {CategoriseData().LiveCourses.map((itr, i) => (
                    <Btn_Batches
                      data={CategoriseData().LiveBatches.filter(
                        (obj) => obj["courseId"] == itr
                      )}
                    />
                  ))}
                </div>
              ...
              <div className="teit_all">
                ...
                <div className="box_vd_s_continer">
                  {CategoriseData().UpcomingCourses.map((itr, i) => (
                    <Btn_Batches
                      data={CategoriseData().UpcomingBatches.filter(
                        (obj) => obj["courseId"] == itr
                      )}
                    />
                  ))}
